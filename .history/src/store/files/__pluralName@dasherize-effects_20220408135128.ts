...
import { Feature<%= classify(pluralName) %>Actions } from './planes-actions';
    ...
    this.actions$.pipe(
      ...
      switchMap((event) =>
        this.planeDas.getListByPost({ event: event }).pipe(
          map((result: DataResult<Plane[]>) => Feature<%= classify(pluralName) %>Actions.loadAllByPostSuccess({ result: result, event: event })),
          catchError((err) => {
    ...
    this.actions$.pipe(
      ...
      switchMap((id) => {
        return this.planeDas.get({ id: id }).pipe(
          map((plane) => Feature<%= classify(pluralName) %>Actions.loadSuccess({ plane })),
          catchError((err) => {
            this.biaMessageService.showError();
            return of(Feature<%= classify(pluralName) %>Actions.failure({ error: err }));
          })
