...
function flattenSchema(
  ...
  const props = []
  const [key, value]: [string, OpenAPIV3_1.SchemaObject] = Object.entries(schema).map(([key, value]) => [key, value as OpenAPIV3_1.SchemaObject]);
  for (let i = 0; i < .length; i++){
    const property = 
  }
  for (const [key, value] of Object.entries(schema)) {
    if (value instanceof ) console.log(key)
    console.log(value)
    console.log(parent)
  }
}
