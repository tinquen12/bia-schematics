...
function getResponseProperties(
  ...
  return Object.entries(object).map(([key, value]: [string, OpenAPIV3_1.]) => )
}

function flattenSchema(schema: OpenAPIV3_1.SchemaObject, parent: string = ""): Property[] | void {
  const props = []
  for (const [key, value]: [string, OpenAPIV3_1.SchemaObject] of Object.entries(schema)) {
    if (value.)
  }
}
