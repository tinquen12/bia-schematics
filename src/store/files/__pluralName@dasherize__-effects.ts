...
export class <%= classify(pluralName) %>Effects {
  loadAllByPost$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('event'),
      switchMap((event) =>
        this.<%= dasherize(name) %>Das.getListByPost(event).pipe(
  ...
  load$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('id'),
      switchMap((id) => {
        return this.<%= dasherize(name) %>Das.get(id).pipe(
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([<%= dasherize(name) %>, event]) => {
        return this.<%= dasherize(name) %>Das.post(<%= dasherize(name) %>).pipe(
          map(() => {
  ...
  update$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((<%= dasherize(name) %>) => of(<%= dasherize(name) %>).pipe(withLatestFrom(this.store.select(getLastLazyLoadEvent)))),
      switchMap(([<%= dasherize(name) %>, event]) => {
        return this.<%= dasherize(name) %>Das.put(<%= dasherize(name) %>, <%= dasherize(name) %>.id).pipe(
          map(() => {
  ...
  destroy$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('id'),
      concatMap((id: <%= idType %>) => of(id).pipe(withLatestFrom(this.store.select(getLastLazyLoadEvent)))),
      switchMap(([id, event]) => {
        return this.<%= dasherize(name) %>Das.delete(id).pipe(
          map(() => {
  ...
  multiDestroy$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('ids'),
      concatMap((ids: <%= idType %>[]) => of(ids).pipe(withLatestFrom(this.store.select(getLastLazyLoadEvent)))),
      switchMap(([ids, event]) => {
        return this.<%= dasherize(name) %>Das.deletes(ids).pipe(
          map(() => {
            this.biaMessageService.showDeleteSuccess();
            if (useSignalR) {
              return biaSuccessWaitRefreshSignalR();
