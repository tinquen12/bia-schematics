...
export function addSymbolToNgModuleMetadata(
  ...
  if (elements.length) {
    const symbolsArray = elements.map((node: any) => tags.oneLine`${node.getText()}`);
...
export function addRouteDeclarationToModule(
  ...
  if (ts.isArrayLiteralExpression(routesArg)) {
    ...
    if (routesArg.kind === ts.SyntaxKind.Identifier) {
      routesVar = source.statements.filter(ts.isVariableStatement).find((v: any) => {
  ...
  const {insertPos, route} = getRoute(source, routesArr, routeLiteral)

  return new InsertChange(fileToAdd, insertPos, route);
}

function getRoute(source: ts.SourceFile, routesArr: ts.ArrayLiteralExpression, routeLiteral: string): {route: string, insertPos: number} {
  ...
  if (occurrencesCount > 0) {
    ...
    // Go take the children route
    const emptyPath = routesArr.elements.find((e: any) => e.properties && e.properties.some(
      (n: any) =>
        ts.isPropertyAssignment(n) &&
        ts.isIdentifier(n.name) &&
        n.name.text === 'path' &&
    ...
    // Add to the children of the empty path
    if (emptyPath && ts.isObjectLiteralExpression(emptyPath)){
      return innerRoute(source, emptyPath, routeLiteral)
    }
  ...
  return ({
    route: routeLiteral,
    insertPos: routesArr.elements.pos,
  })
}

function innerRoute(source: ts.SourceFile, emptyRoute: ts.ObjectLiteralExpression, routeLiteral: string): {route: string, insertPos: number} {
  const childrenRoutes = emptyRoute.properties.find(e => e.name?.getText() === "children") as ts.PropertyAssignment

  return getRoute(source, childrenRoutes.initializer as ts.ArrayLiteralExpression, routeLiteral)
}

export function appendToPermissionsEnum(source: ts.SourceFile, fileToAdd: string, enumLiteral: string) : Change {
  if (!source || !source.statements)
    throw new SchematicsException("There are not Permission enum.")

  const permissionEnum = source.statements.filter(ts.isEnumDeclaration).find((e) => e.name.getText() === "Permission")
  if (!permissionEnum)
    throw new SchematicsException("There are not Permission enum.")
  
  const text = permissionEnum.getFullText()
  const occurenceCount = permissionEnum.members.length

  if (occurenceCount > 0){
    const indentation = text.match(/\r?\n(\r?)\s*/) || [];
    const enumText = `${indentation[0] || ' '}${enumLiteral}`;
    const lastMember = [...permissionEnum.members].pop() as ts.EnumMember;
    return new InsertChange(fileToAdd, lastMember.end, `,${enumText}`)
  }

  return new NoopChange()
}

export function addNavigation(source: ts.SourceFile, fileToAdd: string, navigationLiteral: string) : Change {
  if (!source || !source.statements)
    throw new SchematicsException("There are no NAVIGATION variable.")

  const navigationVar = source.statements.filter(ts.isVariableStatement).find((v: any) => {
    return v.declarationList.declarations[0].name.getText() === "NAVIGATION";
  });
  if (!navigationVar)
    throw new SchematicsException("There are no NAVIGATION variable.")

  const navigationArr = findNodes(
    navigationVar,
    ts.SyntaxKind.ArrayLiteralExpression,
    1,
  )[0] as ts.ArrayLiteralExpression;

  const occurrencesCount = navigationArr.elements.length;
  const text = navigationArr.getFullText(source);

  if (occurrencesCount > 0) {
    const lastNavLiteral = [...navigationArr.elements].pop() as ts.Expression;

    const indentation = text.match(/\r?\n(\r?)\s*/) || [];
    const navText = `${indentation[0] || ' '}${navigationLiteral}`;
    return new InsertChange(fileToAdd, lastNavLiteral.end, `,${navText}`)
  }
